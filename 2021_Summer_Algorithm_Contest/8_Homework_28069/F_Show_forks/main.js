...
function main() {
  ...
  class BSTnode {
    ...
    static showForks(node) {
      let values = []

      if (node.left !== null) {
        values.push(...BSTnode.showForks(node.left))
      }
      ...
      if (node.right !== null) {
        values.push(...BSTnode.showForks(node.right))
      }
  ...
  fs.writeFileSync("output.txt", BSTnode.showForks(root).join('\n'))
}
