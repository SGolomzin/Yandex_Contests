...
function main() {
  ...
  class BSTnode {
    ...
    static showBranches(node) {
      let values = []

      if (node.left !== null) {
        values.push(...BSTnode.showBranches(node.left))
      }
      ...
      if (node.right !== null) {
        values.push(...BSTnode.showBranches(node.right))
      }
  ...
  fs.writeFileSync("output.txt", BSTnode.showBranches(root).join('\n'))
}
